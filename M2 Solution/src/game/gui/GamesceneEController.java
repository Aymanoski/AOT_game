...
public class GamesceneEController {
	...
	public void addtitan(AnchorPane l) {
		
		l.getChildren().clear();
		if (l==L) {
			PriorityQueue<Titan> t= b.getOriginalLanes().get(0).getTitans();
			while(!t.isEmpty()) {
		TitanView selectedTitan = new TitanView(t.peek());
		AnchorPane.setLeftAnchor(selectedTitan,(double)t.poll().getDistance()*8);
		AnchorPane.setTopAnchor(selectedTitan, 0.0);
		AnchorPane.setBottomAnchor(selectedTitan, 0.0);
		L.getChildren().add(selectedTitan);}}
		if (l==L1) {
			PriorityQueue<Titan> t= b.getOriginalLanes().get(1).getTitans();
			while(!t.isEmpty()) {
		TitanView selectedTitan = new TitanView(t.peek());
		AnchorPane.setLeftAnchor(selectedTitan,(double)t.poll().getDistance()*8);
		AnchorPane.setTopAnchor(selectedTitan, 0.0);
		...
		L.getChildren().add(selectedTitan);}}
		if (l==L2) {
			PriorityQueue<Titan> t= b.getOriginalLanes().get(2).getTitans();
			while(!t.isEmpty()) {
		TitanView selectedTitan = new TitanView(t.peek());
		AnchorPane.setLeftAnchor(selectedTitan,(double)t.poll().getDistance()*8);
		AnchorPane.setTopAnchor(selectedTitan, 0.0);
